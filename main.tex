...
The fixed-makespan \mapfr problem is the following decision problem: given a \mapfr problem $P$ and a makespan bound $\mu$, is there a solution to $P$ whose makespan is at most $\mu$? 
Next, we propose an algorithm that solves this problem by following the standard \smt solving procedure described above. 
A satisfying assignment to the \ps in our algorithm defines a solution to the given \mapfr problem whose makespan is smaller than or equal to $\mu$. 
The \decidet procedure in our algorithm applies a conflict detection mechanism to check if that solution is valid. 
If not, it returns one of the detected \ccbs conflict. 
In such cases, the \ps is subsequently updated so that 
satisfying assignments to it will resolve the detected conflict by satisfying one of the corresponding \ccbs constraints. 
...
$\Psi$ contains the set of \ccbs conflicts detected so far, which is initially empty (line~\ref{line:smt:init}). 
In every iteration, the \ps is created for the set of previously-found conflicts (CreatePS($\Psi$) in line~\ref{line:smt:createps}). 
...
Otherwise, \decidet returns a \ccbs conflict, which is added to the set of conflicts $\Psi$. 
