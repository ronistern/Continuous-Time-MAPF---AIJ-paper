...
Our SMT-based algorithm for solving the fixed-makespan problem mimics to the behavior of \ccbs in that it detects \ccbs conflicts and resolves them by imposing of \ccbs constraints that are computed in the same way as in \ccbs. It follows the standard SMT solving procedure described above with a \ps and a \decidet procedure. 
The \ps defines a solution to the given \mapfr problem, while the \decidet applies a conflict detection mechanism to check if that solution is valid. If not, it returns one of the detected \ccbs conflict. 
Throughout its execution, our algorithm maintains a set of all the previously detected \ccbs conflicts. We call this set the \emph{active conflicts} set. 
Our \ps is constructed in such a way that a satisfying assignment to it 
defines a solution to the given \mapfr problem that resolves the set of active conflicts and has a makespan smaller than or equal to the makespan bound $\mu$. 
...
$\Psi$ contains the set of active conflicts, which is initially empty (line~\ref{line:smt:init}). 
In every iteration, a \ps is created for the set of active conflicts (line~\ref{line:smt:createps}). 
...
Otherwise, \decidet returns a \ccbs conflict, which is added to the set of active conflicts. 
This process continues until either a valid solution is found (line~\ref{line:smt:solution}) or we establish that no solution exists (line~\ref{line:smt:no-solution}).
 ...
 which is then incorporated into the \ps by adding appropriate \ccbs constraints. 


% Active conflicts
Throughout its execution, our algorithm maintains a set of active \ccbs conflicts. 
This set is initially empty and new \ccbs conflicts are added to it after every invocation of our \decidet procedure. 
Our \ps is constructed in such a way that a satisfying assignment to it 
defines a solution to the given \mapfr problem that resolves the set of active conflicts and has a makespan under the makespan bound $\mu$. 


We show below how this is done. 
