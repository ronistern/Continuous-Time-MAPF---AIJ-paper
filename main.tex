...
Otherwise, some \ccbs conflict $C=\tuple{i,j, (a_i,t_i), (a_j,t_j)}$ detected for $N.\Pi$ is chosen. 
...
The corresponding constraints for agent $i$ and $j$ are $\tuple{i,[t_i,t_i^u)}$ and
$\tuple{j,[t_j,t_j^u)}$, denoted $C_i$ and $C_j$, respectively.  
