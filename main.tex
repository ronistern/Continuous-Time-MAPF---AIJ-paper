...
Our SMT-based algorithm for solving a fixed-makespan problem mimics to the behavior of \ccbs by growing an implicit version of the \ccbs Constraint Tree (\ct). 
This implicit \ct represents only the leaves of the \ccbs Constraint Tree 
...
To generate our \ps for an implicit \ct, we compute for each agent $a_i$ 
every single-agent plans that \csipp may return if applied to every node in the implicit \ct
