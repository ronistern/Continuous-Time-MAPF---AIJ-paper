...
The CreatePS procedure mimicks \ccbs as follows. 
For a \ccbs conflict $C$, let $Cons(C)$ denote the pair of \ccbs 
constraints used by \ccbs to resolve $C$ (defined in Lemma~\ref{lem:sound}). 
For a constraint $c\in Cons(C)$, we denote by $c(\Pi)$ the proposition that constraint $c$ is satisfied in a solution $\Pi$. 
Observe that a \mapfr solution $\Pi$ does not include any conflicts from $\Psi$ iff at least one of the constraints in $Cons(C)$ for every \ccbs conflict $C\in\Psi$. This translates to the following 
\begin{equation}
\text{$\Pi$ has no conflicts from $\Psi$}
\leftright
\bigwedge_{C\in \Psi} \Big(\bigvee_{c\in Cons(C)} c(\Pi) \Big)
\end{equation}

a set of \ccbs constraints that is a hitting set of $Cons(\Psi)$. 


Let $Cons(\Psi)_i$ be the set of \ccbs constraints that are defined for agent $a_i$. 



it creates a pair of \ccbs constraints as defined in Lemma~\ref{lem:sound}. 
Let $Cons(C)$ denote this pair of \ccbs constraints 
and $Cons(\Psi)=\{ Con(C) | C\in \Psi \}$. 
Note that $Cons(\Psi)$ is a set of pairs of \ccbs constraints. 
Due to Lemma~\ref{lem:sound}, every \mapfr solution that does not include any conflict in $\Psi$ must satisfy a set of \ccbs constraints that is a hitting set of $Cons(\Psi)$. 


Based on this observation, we compute for every agent $a_i$ the set of single-agent plans that \csipp may return given a hitting set of constraints. 



. 
%This pair of \ccbs constraints is known to be sound (in the sense of Definition~\ref{def:sound}), and thus every solution that does not have the conflict $C$ must satisfy one of the \ccbs constraints in $Cons(C)$. 
Due to Lemma~\ref{lem:sound}, we have that every \mapfr solution that does not include any conflict in $\Psi$ must satisfy 
for every $C\in\Psi$ one of the \ccbs constraints in $Con(C)$. 
In other words, every such \mapfr solution 
satisfies a \emph{hitting set} of $\{ Con(C) | C\in \Psi \}$. 





More generally, we define our \ps such that an assignment that satisfies it
defines a \mapfr solution that satisfies for every $C\in \Psi$ one of the \ccbs constraints in $Con(C)$. 
