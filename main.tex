...
% Outline of section
In this section, we propose an alternative compilation-based algorithm for finding makespan-optimal solution to \mapfr problems. This algorithm is called \smtcbs

based on \emph{SAT modulu Theory (SMT)}~\cite{DBLP:journals/jacm/NieuwenhuisOT06,DBLP:journals/constraints/BofillPSV12,DBLP:conf/cp/Nieuwenhuis10}. We start by providing a brief background on SMT. 
...
% We start by converting the optimization problem to a decision problem 
We p



A key component in our SMT-based algorithm for solving \mapfr is an algorithm that solves the following decision problem: given a \mapfr problem $P$ and a makespan bound $\mu$, is there a solution to $P$ whose makespan is at most $\mu$? 
We call this decision problem the \emph{fixed-makespan \mapfr problem}.  



% High-level: we grow an implicit CT
The algorithm we propose for solving the fixed-makespan problem mimics to the behavior of \ccbs in that it detects \ccbs conflicts and resolves them by imposing of \ccbs constraints. 
This algorithm follows the standard SMT solving procedure described above, 
iteratively creating a \ps, solving it, and using a \decidet procedure to check if it satisfied the underlying non-Boolean logic. 
This \ps defines a solution to the given \mapfr problem, while the \decidet applies a conflict detection mechanism to check if that solution is valid. If not, it returns one of the detected \ccbs conflict. 
...
\begin{algorithm}
	\SetKwInOut{Input}{input}
	\Input{($\mathcal{G}=(V,E)$, \source, \target,$\mu$)}
