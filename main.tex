...
\newcommand{\mddr}{\ensuremath{MDD_R}}
\usepackage{amsmath}
\usepackage{amsthm}
\usepackage{amssymb}
...
\begin{algorithm}
\SetKwBlock{CCBS}
{\ccbs($\mathcal{G}=(V,E)$, \source, \target)}{end} 
CCBS{
    \ForEach{agent $i$}{
...
Next, we propose an SMT-based algorithm for solving the following decision problem: given a \mapfr problem $\Pi$ and a makespan bound $\mu$, is there a solution to $\Pi$ whose makespan is at most $\mu$? 
We call this decision problem the \emph{fixed-makespan \mapfr problem}.  


% High-level: we grow an implicit CT
Our SMT-based algorithm for solving a fixed-makespan problem mimics to some extent the behavior of \ccbs by \emph{implicitly growing the \ct} of \ccbs. This \ct is implicit in the sense that it each \ct node contains \ccbs constraints but it does not contain a solution that satisfies them. That is, every node $N$ in the implicit \ct is defined by $N.\const$ and not $N.\pi$. 

% High-level: the PS is created to find a solution for the implicit CT
Our \ps is created such that a satisfying assignment to the \ps defines a \mapfr solution $\pi$ that is consistent with the constraints of some leaf $N$ of the implicit \ct. 
The \decidet procedure is analogues to checking if $N$ is a goal \ct node. That is, \decidet checks if $\pi$ is valid, i.e., if it represents a set of single-agent plans that do not conflict. 
If $\pi$ is not a valid solution, our \decidet returns a corresponding \ccbs conflict. 
Then, we extend the implicit \ct by expanding $N$ according to this \ccbs conflict as done by \ccbs. 
The \ps is updated to reflect the change in the implicit \ct. 
...
%\SetKwIBlock{($\mathcal{G}=(V,E)$, \source, \target,X)}
    $N\gets\emptyset$\\
    Create LEAVES; Add $N$ to LEAVES \nllabel{line:smt:init}\\
    \While{LEAVES is not empty}{
        \ps $\gets$ CreatePS(Leaves) \nllabel{line:smt:createps}\\
        $(N,\pi)\gets$ Solve(\ps) \nllabel{line:smt:solve}\\
        \If{No solution found to \ps }{
        ...
        Remove $N$ from LEAVES\\        
        $N_1, N_2\gets$ expand $N$ according to $Con$\\
        Add $N_1$ and $N_2$ to LEAVES\\
    }
...
Algorithm~\ref{alg:smt-ccbs} lists a high-level pseudo-code for our SMT-based algorithm. The LEAVES datastructure contains the set of leaf nodes of our implicit \ct. 
Initially, LEAVES contain a node with an empty set of constraints (line~\ref{line:smt:init}). 
In every iteration, a \ps is created for the current set of \ct leaves (line~\ref{line:smt:createps}). 
...
Otherwise, \decidet returns a \ccbs conflict and we create two new leaf \ct nodes by adding \ccbs constraints to resolve this conflicts. 

We describe this SMT-based algorithm in detail below.
...
To generate our \ps for an implicit \ct, we compute for each agent $a_i$ all single-agent plans that \csipp may return for it in one of the leaf nodes of the implict \ct. 



that represents a breadth-first search over all single-agent plans for $a_i$ having cost lower than the fixed makespan bound $\mu_{max}$ that may satisfy the set of \ccbs constraints. \roni{This is not accurate. There is an infinite number of such single-agent plans. So, I'm not sure how to exactly define what this \mddr represents.} \konstantin{And I didn't get the phrase once again :) Aren't some words missing in it?}
We store this set of single-agent plans in a compact data structure called
\emph{Multi-Value Decision Diagram} (MDD)~\cite{srinivasan1990algorithms}. 
An MDD is a direct a-cyclic graph with a single source and sink.\footnote{Technically, an MDD can have multiple sinks, where each sink is labelled as either true or false. This is equivalent to having a single sink that gathers all sinks labelled as true, and removing all branches that only end up in sinks labelled as false.}
...
A node in the \mddr is a pair $(u,t)$ where $u$ is a vertex in $\mathcal{G}$ and $t$ is a point in time. 
Generating this \mddr is done by performing a breadth-first search (BFS), starting from $(\source(i),0)$. 
Expanding a node $(u,t)$ involves generating a new node $(v,t+a_D)$ 
for every move action $a$ that moves the agent from $u$ to $v$, as long as it ends before the makespan bound, i.e., when $t+a_D\leq \mu_{max}$. 
We create the new node $(v,t+a_D)$
This includes move actions that conflict with the given set of \ccbs constraints. 
In such cases, i.e., when a node $(u,t)$ has an action $a$ 
If such a move action 
Two types of actions are used: {\em edge traversals} and {\em waiting}. 
The edge traversal is the standard operation from BFS; having node $(u,t)$ at hand we create a new node $(v,t+a_D)$ for every 
action $a$ that moves the agent from $u$ to $v$ (line 12). 
Nodes and corresponding edges resulting from this expansion are added to \mddr (lines 15-16). 
A wait action 


For each constraint forbidding agent $i$ to traverse from $(u,v)$ at time $t$, we allow the agent to wait at $u$ from $t$ to the end of the constraint's time interface $[t_i,t_i^u]$, we add a wait action in $u$ until $t_i^u$ is performed and RDD is extended with wait nodes and edges (lines 20-21). Consequently each conflict during the RDD generation process through BFS is treated as both present and absent which in effect generates all possible important moments.
