...
The CreatePS procedure generates a \ps such that a satisfying assignments to it
defines a solution in which all the \ccbs conflicts in $\Psi$ are avoided. 
Just like the SAT-MDD encoding, our \ps has two Boolean variables -- $\mathcal{X}_{v}^{t}(a_i)$ and
$\mathcal{E}_{v,v'}^{t,t'}(a_i)$. Setting $\mathcal{X}_v^t(a_i)$ to true represents that agent $a_i$ is at location $v$ in time $t$. Setting $\mathcal{E}_{v,v'}^{t,t'}(a_i)$ to true represents that agent $a_i$ performs an action that start at time $t$, ends at time $t'$, and moves the agent from $v$ to $v'$. 
Wait actions are represented by $\mathcal{E}_{v,v'}^{t,t'}(a_i)$ variables with $v=v'$. 
Our \ps includes the following constraints over these variables to ensure that an assignment to the variables that satisfy these constraints is a solution to the given \mapfr problem. 
\begin{equation}
...
CreatePS consists of three steps. 
In the first step, it computes the possible sets of \ccbs constraints that \ccbs would impose to resolve the conflicts in $\Psi$, based on Lemma~\ref{lem:sound}. 
In the second step, it creates a propositional formula that represents all possible single-agent plans \csipp would return for each of the agents that satisfy these constraints.
In the third and final step, it adds to this formula additional constraints to verify that the set of single-agent plans selected avoided all conflicts by satisfying for every conflict in $\Psi$
one of its respective \ccbs constraints. 



\noindent \textbf{Step \#1. Computing \ccbs constraints for every conflict.}
For every \ccbs conflict $C$ in $\Psi$, we generate the pair of \ccbs constraints \ccbs would create to resolve $C$, as specified in Lemma~\ref{lem:sound}. This requires computing the unsafe intervals for each conflict. 
...
To create the \mddr for each agent $a_i$, we generate all children of all nodes in a breadth-first manner, starting from the root node $(\source(i),0)$, which represents agent $a_i$ in its start location at time 0. For a pseudo code of this \mddr generation process, see~\ref{sec:code-mddr}. 


% From MDDR to propositional formula
