...
Let $Cons(\Psi)$ be the set of \ccbs constraints pairs for all the constraints in $\Psi$, i.e., $Cons(\Psi)=\{ Con(C) | C\in \Psi \}$. 
Observe that every \mapfr solution that does not include any conflicts from $\Psi$ must satisfy at least one of the constraints in $Cons(C)$ for every \ccbs conflict $C\in\Psi$. This translates to the following propositional. 
\begin{equation}
\bigwedge_{C\in \Psi} \Big(\bigvee_{c\in Cons(C)} c \Big)
\end{equation}
