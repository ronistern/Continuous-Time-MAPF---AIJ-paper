...
We denote by the set of edges and nodes in the \mddr by $E^i$ and $X^i$, respectively. 

\roni{Pavel had some visual examples of \mddr.}


\noindent \textbf{Step \#3. Create propositional formula.}
For every node $(v,t)$ and edge $\big((v,t), (v',t')\big)$ in the \mddr of each agent $a_i$ 
we create the Boolean variables $\mathcal{X}_{v}^{t}(a_i)$ and
$\mathcal{E}_{v,v'}^{t,t'}(a_i)$, respectively. 
...
Our \ps includes the following constraints over these Boolean variables to ensure that an assignment to them is a solution to the given \mapfr problem. 
\begin{equation}
