...
The CreatePS procedure generates a \ps that mimicks the behavior in \ccbs. 
It considers the possible sets of constraints that \ccbs would impose to resolve the conflicts in $\Psi$, and it considers the possible single-agent plans \csipp would find to satisfy these constraints. This is done as follows. 
...
Let $Cnst_i$ be the set of all \ccbs constraints on agent $a_i$ that were generated this way, i.e., 
\begin{equation}
Cnst_i = \{ \tuple{i, a_i, [t_i, t_i^u)} | 
