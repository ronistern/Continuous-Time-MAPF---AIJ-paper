...
This implicit \ct is defined by a set of \ccbs conflicts $(C_1,...,C_n)$. 
The tree has $i$ levels, where every level represents one of these conflicts. 
Every outgoing edge from a node in a level for a conflict $\tuple{i,j, (a_i, t_i), (a_j, t_j)}$ 
represents one of the two \ccbs constraint that can resolve it: 
$\tuple{i, ]

Note that for any \ccbs conflict   
there is exactly 
