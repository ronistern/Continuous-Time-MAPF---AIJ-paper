...
Next, we propose an algorithm called \emph{\mu\smtcbs}

that solves this problem by following the standard \smt solving procedure described above. 
...
\begin{algorithm}
	...
	\caption{The Fixed-\smtcbs algorithm.}
	\label{alg:smt-ccbs}
\end{algorithm}
...
\begin{algorithm}[h]
		...
		\Input{$\mathcal{G}=(V,E)$, \source, \target)}
			$\Psi \gets \emptyset$\\
			$\Pi \gets$ $\{\pi_i$ a shortest  plan from $\source(i)$ to $\target(i)\;|\;i = 1,2,...,k\}$ \\
