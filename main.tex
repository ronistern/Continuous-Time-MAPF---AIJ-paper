...
The \decidet procedure in $\mu$\smtcbs checks if this solution is valid, i.e., if it contains any \ccbs conflicts. 
...
$\mu$\smtcbs maintains a set $\Psi$ that contains all the \ccbs conflicts returned by our \decidet procedure so far. 
...
that conflicts with agent $a_i$ performing $a$ at time $t$, 
...
Importantly, this child node is not added to $(v,t)$ instead of $(v', t+a_D)$. 
...
\roni{Pavel had some visual examples of \mddr.}


\noindent \textbf{Step \#3. Create a propositional formula.}
Let $E^i$ and $X^i$ denote the set of edges and nodes, respectively, in the \mddr of agent $a_i$. 
We create the Boolean variables $\mathcal{E}_{v,v'}^{t,t'}(a_i)$ and $\mathcal{X}_{v}^{t}(a_i)$ 
for every edge and node in $E^i$ and $X^i$ respectively. 
Setting $\mathcal{X}_v^t(a_i)$ to true represents that agent $a_i$ plans to occupy location $v$ in time $t$. 
Setting $\mathcal{E}_{v,v'}^{t,t'}(a_i)$ to true represents that agent $a_i$ plans performs an action that start at time $t$, ends at time $t'$, and moves the agent from $v$ to $v'$. 
Note that wait actions are represented by $\mathcal{E}_{v,v'}^{t,t'}(a_i)$ variables with $v=v'$. 
...
\label{eq-3}
\end{equation}
The constraints in Equations~\ref{eq-1} and~\ref{eq-2} state that if agent $a_i$ appears in vertex $v$ at time $t$ then it has to leave through exactly one edge connected to $v$ (waiting included). 
...
We also add constraints that verify every agent starts in its start location and ends in its goal. \roni{Pavel, is the last sentence correct?}
...
To avoid all the \ccbs conflicts in $\Psi$, we verify that for every conflict $C\in\Psi$ one of the \ccbs constraints in $Cnst(C)$ is satisfied. 
...
where $a_i$ moves from $v$ to $v'$ 
and $a_j$ moves from $u$ to $u'$, 
we add following clause:
\begin{equation}
	\mathcal{E}^{t_i, t_i+{a_i}_D}_{v,v'}(i)
\vee
	\mathcal{E}^{t_j, t_j+{a_j}_D}_{u,u'}(j)
\end{equation}
The resulting propositional formula is used in our algorithm as the \ps. 
...
The correctness of this algorithm relies on the fact that the solvability of \mapfr w.r.t. cumulative objective like the makespan behaves as a non decreasing monotonic function. Hence trying increasing makespans eventually leads to finding the optimal makespan provided we do not skip any relevant makespan $\mu$. The next makespan to try will then be obtained by taking the current makespan plus the smallest duration of the continuing movement. The iterative scheme for trying larger makespans follows MDD-SAT \cite{DBLP:conf/ecai/SurynekFSB16}.
