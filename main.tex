...
\caption{Generate \mddr for single agent $i$.} \label{alg:generate-mddr}
\end{footnotesize}
\end{algorithm} 
...
%$\tuple{i, (v,v'), [t_i, t_i^u)}$ in $Const(\Psi)$ 
%$\tuple{i, (v',v'), [t, t')}$ in $Const(\Psi)$ 
%\roni{Pavel, in this case, do we add some new node or none is needed?} \pavel{Yes, the node is created but constraints may forbid its usage.}
% This is original paragraph replace by the alternative description below
...
%\pavel{The next makespan to try $\mu_{next}$ is calculated as the minimum of end times of move actions $a \in A$ that starts before the makespan bound but ends after (i.e., $\mu_{next} = \min_{(a,t) \in A}\{t + a_D, t \leq \mu \wedge t + a_D > \mu$\}). }\konstantin{I think, I'm starting to get what Pavel is trying to say, but the formula is wrong, I believe. Btw, do I understand right that the move actions that span beyond the current makespan bound are nod added to \mddr (we said this in the beginning of the above paragraph?} \pavel{What is wrong in the formula? Changed $a \in A$ to $(a,t) \in A$, maybe this was wrong.} \pavel{Answer: actions going beyond makespan bound are not added to \mddr.}
%\konstantin{In the paragraph above we did not specify how do we handle constraints imposed over the wait-actions. Suppose, the wait action $a$ is constrained via the $\tuple{i, (v,v), [t_i, t_i^u)}$. How is it reflected in \mddr?}\roni{Good call. I think that we need to add at the end of the paragraph above the following: ``Every child $(v,t')$ of a node $(v,t)$ represents a wait action. We consider wait actions in a similar way that were described above for move actions. If there is a constraint that forbids this wait action, we add additional nodes to the \mddr that satisfy this constraints.''  Pavel - can you verify/edit/add to above an answer to Konstantin?} \pavel{I think this is correct explanation. The wait action is treated much like move action, that is, degenerate version of move action. Maybe we need to define how to compute unsafe interval for the wait action if we want to give more details. } \konstantin{Guys, I realize now that the right question to ask is the following: to create a \mddr for an agent $i$ does SMT-CBS actually run CSIPP for this agent with every subset of $i$th constraints and then just represents the obtained solutions in the form of \mddr? If the answer is YES, than no special handling of the constraints imposed over the wait actions is needed at the \mddr level (they are handled by CSIPP).}
%\roni{I don't think this is what happens, because that would mean running \csipp for every subset of constraints, and then aggregating all possible paths. So, the process of generating the \mddr is more efficient. }
%\roni{I added text to discuss this wait action issue. There's also a question there for Pavel}
%\pavel{Today I understood the issue with the wait action differently, still not sure if my understanding is correct. But if there is a constraint over the wait action we cannot resolve it by more waiting. This is rather a dead end (assume waiting in v from 2.0 to 3.0 but constraint say we cannot wait in v from 2.5 to 4.0, this is dead end or not? Resolving here is only not to use the first wait action which is already covered in different branches of \mddr). I would hence not add the sentence at the end of the paragraph ''Every child node ...''}
%\pavel{\mddr generation code back here. The code is from early version, uses different notation. The idea is breadth-first search (Dijkstra), having node (u,t) u - vertex, t - time, we do everything possible in that node: (i) make an action ignoring constraints (lines 13-16), (ii) for that action, check constraints across the edge used by the action and generate wait action to overcome the constraint (lines 17-21). Each new action generates a state that is further expanded.}


%\roni{I have some issues with the pseudo code in Alg.~\ref{alg-DEC-gen}.  Assume the initial location is A,  and it has an neighbor B, and the distance between them is 10.  Now assume that there are two constraints: (1) on the action "move from A to B" for which $[t_i,t_i^u]$ is $[0,5]$, and (2) on the action "wait at A" for which $[t_i,t_i^u]=[2,3]$.  From the code above, I think it would ignore the second constraint.}



%\pavel{I changed the alternative version a bit - now it is alternative version 2. (1) I think there was a typo in waiting time w.r.t. wait action in the target of a move: wait time  + duration should give $t'^u$. (2) We can make the algorithm more stupid by removing time interval checks before introducing wait action. Now we enumerate across all safe/unsafe intervals given by constraints over action $a$ and generate waits for all end-times of unsafe intervals. Now I think that this corresponds to "doing everything possible in the node". Note: I think we do not need any type of backpropagating waits in the algorithm. Single-path finding algorithms are polynomial by nature which backtracking would contradict. }
%\roni{Fixed typo, need to think about the other stuff}




% This seems to be old attempt
\begin{comment}
Next, we establish the correctness of $\mu$\smtccbs. 
A single-agent plan for agent $i$ is called an \emph{optimal constrained single-agent plan} w.r.t. a set of constraints $Const$ iff there is no other single-agent plan that satisfies $Const$ and has a smaller cost. 
The first step to establish the correctness of $\mu$\smtccbs is the following Lemma. 
\begin{lemma}
The \mddr created by Algorithm~\ref{alg:alternative-mddr} contains at least one optimal-constrained single-agent plan for any given subset of constraints in $Const$. 
\label{lem:mddr-correct}
\end{lemma}
\begin{proof}
Let $\mddr^i=(X^i,E^i)$ be an \mddr generated by
Algorithm~\ref{alg:alternative-mddr} and let $Const'$ be a subset of $Const$, i.e., $Const' \subseteq Const$. 
We prove Lemma~\ref{lem:mddr-correct} by induction over the size of $Const'$. 

Consider first $Const'$ that contains a single constraint. 
If the constrain is over a wait action, then the 

$\mddr^i$ 

By construction, \mddr contains all single-agent plans 
that consists of only move actions and have a duration less than or equal to $\mu$. 

[[TODO: Now we need to prove that this covers all options]]
\end{proof}
\end{comment}



%------ beginning of correctness from the appendix
...
%\konstantin{Some notation remarks. (1) \ccbs constraints are open from the right side, i.e. $(i, (u, \tov(a)), [t',t'^u))$ not $(i, (u, \tov(a)), [t',t'^u])$. (2) Before in text we denoted \ccbs constraints using $\tuple{}$ not $()$, i.e. $\tuple{i, (u, \tov(a)), [t',t'^u)}$. (3) In text we use $Const$ do denote a set of constraints, but in pseudoce we use $constraints$.}
%\konstantin{I think the algorithm's name should be 'Generate \mddr for a single agent.}  \pavel{Done.}

% \begin{definition}[Complete \mddr]
% We say that an \mddr is complete w.r.t a makespan bound $\mu$ and set of \ccbs constraints $Const$ iff
% \begin{multline}
%  \forall Const'\subseteq Const: 
% \text{Makespan}(\csipp(Const'))>\mu \\
% \vee \csipp(Const')\in \mddr(\mu, Const)     
% \end{multline}
% \end{definition}
...
% \begin{lemma}
% Let $\pi=(a_1, \ldots , a_n)$ be a single-agent plan for agent $i$ 
% returned by \csipp given a set of \ccbs constraints $Const$, 
% and let $t_j$ be the time the $j^{th}$ action in $\pi$ is planned to start. For any wait action $a_j\in\pi$ at location $v$ (i.e., $\fromv(a_j)=\tov(a_j)=v$)
% there exists a \ccbs constraint $\tuple{i, a_C, (t,t^u)}$ such that
% either \\
% (case \#1) $a_C$ is a move action, $\fromv(a_j)=v$, and the duration of $a_j$ is $t^u-t_j$, \\
% or (case \#2) $a_C$ is a wait action at some vertex $v'$, and there exists a move action $a'$ such that $\fromv(a')=v$ and $\tov(a')=v'$, and the duration of $a_j$ is $(t^u-a'_D)-t_j$.
% \end{lemma}
...
To prove Theorem~\ref{the:mddr-ok}, we proof by induction over $j$ that for every timed action $(t_j,a_j)$ in this set there exists an edge $((\fromv(a_j),t_j), (\tov(a_j),t_j+(a_j)_D))$ in our \mddr. 
...
%------ end of correctness from the appendix
...
\SetKwBlock{NRICL}{GenerateMdd$_R$ ($\mathcal{G}=(V,E)$, $\source$, $\target$, $\const(\Psi)$, $\mu$, i)}{end} \NRICL{
        ...
        \While {$\textsc{Open} \neq \emptyset$} {
                ...
                \If {$t \leq \mu$}{
    				...
    				$X^i \gets X^i \cup \{  (\tov(a),t+a_D) \}$ \nllabel{line:generate-mddr:start:move} \\             		 		
    				$E^i \gets E^i \cup \{  [(u,t);(\tov(a),t+a_D)] \}$ \nllabel{line:generate-mddr:end:move} \\             		 						
                			\For {each $\tuple{i, (u, \tov(a)), [t',t'^u)} \in \const(\Psi)$
                			        \nllabel{line:generate-mddr:start:wait1}}{
                				\If {$t\in [t', t'^u]$}{
                     				insert $(u,t'^u)$ into $\textsc{Open}$\\
        						$X^i \gets X^i \cup \{  (u,t'^u) \}$ \\
                			...
                			\For {each $\tuple{i, (\tov(a), \tov(a)), [t',t'^u)} \in \const(\Psi)$
                			        \nllabel{line:generate-mddr:start:wait2}}{
                    				\If {$t<t'^u-a_D$}{
                     				insert $(u,t'^u-a_D)$ into $\textsc{Open}$\\
        						$X^i \gets X^i \cup \{  (u,t'^u-a_D) \}$ \\
...
%\roni{TODO: Completeness?}
%\pavel{Relying on CCBS. I am afraid of infinite convergence like adding constraints for smaller and smaller waitings. And the algorithm will run forever. The same issue is faced in CCBS but seems to be resolved somehow there. Thinking how to avoid this. We need to say that given \mapfr instance can give raise to finite number of wait actions then we eventually try all of them.}
\end{proof}
...
\section{Correctness of the \mddr Generation}
...
\caption{Generate \mddr} \label{alg:generate-mddr}
\end{footnotesize}
\end{algorithm} 
