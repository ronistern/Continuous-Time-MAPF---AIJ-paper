...
Our SMT-based algorithm for solving the fixed-makespan problem mimics to the behavior of \ccbs by growing an \emph{implicit} version of the \ccbs Constraint Tree (\ct). 
This implicit \ct is defined by a set of \ccbs conflicts $(C_1,...,C_n)$. 
Let $Cons(C_i)$ be the pair of \ccbs constraints that \ccbs would impose to resolve $C_i$. That is, for a conflict $\tuple{i,j, (a_i, t_i), (a_j, t_j)}$ we define 
 \begin{equation}
